...
public static class Part2
{
    ...
    public static long Solve(string input)
    {
        ...
        var expectedOutput = new List<int>(program.instructions);

        for (var i = 0;; i++)
        {
            program.registers.a = i;
            if (Execute(program, expectedOutput))
            {
                return i;
            }
        }
    ...
    private static bool Execute(
        ((int a, int b, int c) registers, List<int> instructions) program,
    ...
    )
    {
        ...
        for (var i = 0; i < instructions.Count; i++)
        {
            switch ((InstructionEnum)instructions[i])
            {
                ...
                case InstructionEnum.Out:
                {
                    ...
                    if (expectedOutput.Count <= outputs.Count || expectedOutput[outputs.Count] != val)
                    {
                        return false;
                    }
        ...
        var correct = expectedOutput.SequenceEqual(outputs);
        if (correct)
        {
