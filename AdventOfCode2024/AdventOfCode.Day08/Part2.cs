...
public static class Part2
{
    ...
    )
    {
        ...
        {
            var xIteratedDiff = xDiff * iteration;
            var yIteratedDiff = yDiff * iteration;
            var current = new List<(int x, int y)>();

            if (x1 == x2)
            {
                var newPoint1 = (x1, Math.Min(y1, y2) - yIteratedDiff);
                AddIfValid(current, matrix, newPoint1);
                var newPoint2 = (x1, Math.Max(y1, y2) + yIteratedDiff);
                AddIfValid(current, matrix, newPoint2);
            }
            else if (y1 == y2)
            {
                var newPoint1 = (Math.Min(x1, x2) - xIteratedDiff, y1);
                AddIfValid(current, matrix, newPoint1);
                var newPoint2 = (Math.Max(x1, x2) + xIteratedDiff, y1);
                AddIfValid(current, matrix, newPoint2);
            }
            else
            {
                var xCoef = x1 > x2 ? 1 : -1;
                var yCoef = y1 > y2 ? 1 : -1;
                var newPoint1 = (x1 + xCoef * xIteratedDiff, y1 + yCoef * yIteratedDiff);
                AddIfValid(current, matrix, newPoint1);
                var newPoint2 = (x2 - xCoef * xIteratedDiff, y2 - yCoef * yIteratedDiff);
                AddIfValid(current, matrix, newPoint2);
            }
